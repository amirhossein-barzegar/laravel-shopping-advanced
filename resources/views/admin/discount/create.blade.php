        ...
        <div class="col-start-4 col-span-6 bg-white px-4 py-3 border border-gray-100 rounded">
            <div class="flex flex-col justify-between items-center gap-2 px-4 py-3 sm:flex-row">
                <div class="text-lg text-center sm:text-right font-semibold text-gray-800 bg-white">
                    ...
                    تخفیف مورد نظر خود را در اینجا ثبت کنید
                    </p>
                </div>
            </div>
            <form action="{{ route('discount.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="flex flex-col gap-1 mt-2 ">
                    <label for="amount" class="text-sm font-medium text-gray-900">مقدار تخفیف : </label>
                    <input type="number" min=0 max=100 value="{{ old('amount') }}" name="amount" id="amount" class="outline-none border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-gray-200 transition-all duration-200" required autofocus>
                ...
                <div class="flex flex-col gap-1 mt-2 ">
                    ...
                    <input type="datetime-local" value="{{ old('start_time') }}" name="start_time" id="start_time" class="outline-none border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-gray-200 transition-all duration-200">
                ...
                <div class="flex flex-col gap-1 mt-2 ">
                    ...
                    <input type="datetime-local" value="{{ old('expire_time') }}" name="expire_time" id="expire_time" class="outline-none border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-gray-200 transition-all duration-200">
                    <x-input-error :messages="$errors->get('expire_time')"/>
                </div>
                <div class="flex flex-col gap-1 mt-2">
                    <button class="flex items-center justify-center text-sm gap-2 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 outline-none rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-gray-200 transition-all duration-200">ایجاد تخفیف</button>
                </div>
            </form>
        </div>
    </div>
