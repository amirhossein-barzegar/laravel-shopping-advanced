        ...
        <div class="col-span-4">
            <ul>
                <li class="flex gap-4 text-sm items-center">
                    ...
                    <ul>
                        <li class="flex gap-4 text-sm items-center">
                            <a href="{{ route('brand.index') }}" class="text-gray-600 text-sm">همه برند ها</a>
                            <small class="text-gray-500 text-xs"><i class="fa-solid fa-chevron-left"></i></small> 
                            <ul>
                                <li>
                                    <div class="text-gray-400 text-sm">افزودن برند</div>
                                </li>
                            </ul>
                        </li>
                    </ul>
        ...
        <div class="col-start-4 col-span-6 bg-white px-4 py-3 border border-gray-100 rounded">
            <div class="flex flex-col justify-between items-center gap-2 px-4 py-3 sm:flex-row">
                <div class="text-lg text-center sm:text-right font-semibold text-gray-800 bg-white">
                    ...
                    برند مورد نظر خود را در اینجا ویرایش کنید
                    </p>
                </div>
            </div>
            <form action="{{ route('brand.update', $brand->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="flex flex-col gap-1 mt-2 ">
                    <label for="name" class="text-sm font-medium text-gray-900">نام برند : </label>
                    <input type="text" value="{{ $brand->name }}" name="name" id="name" class="outline-none border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-gray-200 transition-all duration-200" required autofocus>
                    <x-input-error :messages="$errors->get('name')"/>
                </div>
                <div class="flex flex-col gap-1 mt-2 ">
                    <label for="description" class="text-sm font-medium text-gray-900">توضیحات کامل : </label>
                    <textarea name="description" id="description" cols="30" rows="10" class="outline-none border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-gray-200 transition-all duration-200">{{ $brand->description }}</textarea>
                    <x-input-error :messages="$errors->get('description')"/>
                </div>
                <div class="flex flex-col gap-1 mt-2 ">
                    <label for="slug" class="text-sm font-medium text-gray-900">مسیر برند : </label>
                    <input type="text" value="{{ $brand->slug }}" name="slug" id="slug" class="outline-none border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-gray-200 transition-all duration-200" required>
                ...
                <div class="flex flex-col gap-1 mt-2">
                    <button class="flex items-center justify-center text-sm gap-2 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 outline-none rounded-lg focus:ring-2 focus:ring-blue-300 focus:border-gray-200 transition-all duration-200">ویرایش برند</button>
                </div>
            </form>
        </div>
    </div>
